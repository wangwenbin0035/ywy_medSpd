...
import { Col, Modal, Input , Row, Form, Select, DatePicker, Tabs} from 'antd';
...
class AddRefund extends PureComponent{
    ...
    render(){
        let { visible,druglist,tabArr,tabKey,defaultKey} = this.state;
        const { getFieldDecorator } = this.props.form;
        // const expand = display === 'block';
        const {children,supplierList}=this.props;
        let {
            supplierCode,productTime,validEndDate,producerName,licCode
            ,registKey,lot,inStoreCode,inStoreDate,deliveryCode
        }=tabArr;
        let productTimes='';
        let validEndDates='';
        let inStoreDates='';
